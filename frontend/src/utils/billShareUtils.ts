...
export const buildBillPdfBlob = (ticket: PlacedTicket, agencyNameFallback?: string): Blob => {
  ...
  pagesItems.forEach((pItems, pageIndex) => {
    ...
    if (isFirstPage) {
      ...
      // Title: LUCKY10 - BILL RECEIPT
      stream += `BT /F2 14 Tf 0.85 0.70 0.20 rg ${leftX + 14} ${headerY + 48} Td (LUCKY10 - BILL RECEIPT) Tj ET\n`;
    ...
    const footerText = `Page ${pageNum} of ${totalPages}  --  Lucky10 System`;
    stream += `BT /F1 8 Tf 0.5 0.5 0.5 rg 240 32 Td (${escapePdfText(footerText)}) Tj ET\n`;
